...
public class ImageCodeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request.getSession().setAttribute(SystemConstants.IMAGE_CODE, scode);
		String loginId = CookieUtil.readCookie("loginId", request);
		String uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase();
		if(StringUtils.isEmpty(loginId)){
			RedisUtil.set("code:"+uuid,scode);
			RedisUtil.setnx("code:"+uuid,2*60);
			CookieUtil.writeCookie(response,"code:"+loginId,uuid,-1);
		}
        RedisUtil.set("code:"+loginId,scode);
		CookieUtil.writeCookie(response,"code:"+loginId,uuid,-1);
		// 输出图像
		try {
